...
def test_trace_writer_creates_dashboard_compatible_table(tmp_path):
    ...
    try:
        ...
        n = conn.execute("SELECT COUNT(*) FROM traces WHERE timestamp > 0").fetchone()[0]
        assert n == 1
        rows = conn.execute("SELECT * FROM traces ORDER BY timestamp DESC LIMIT 100").fetchall()
        assert len(rows) == 1
...
async def test_engine_writes_to_traces_table_after_run(tmp_path, monkeypatch):
    ...
    agent = Agent(name="trace_test", llm="openai/gpt-4o-mini", instructions="be brief")
...
async def test_code_python_cwd_is_isolated(tmp_path, monkeypatch):
    ...
    out = await code_execute(
        "import os; print('CWD=' + os.getcwd()); print('FILES=' + str(os.listdir('.')))",
        language="python",
    )
