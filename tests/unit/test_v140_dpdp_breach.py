...
def test_detector_emits_no_indicators_below_threshold():
    from largestack._compliance.dpdp_breach import (
        BreachDetector, BreachDetectorConfig,
    )
...
def test_detector_drops_old_window_entries():
    ...
    for _ in range(15):
        detector.observe_read(
            tenant_id="t1", user_id="u1", timestamp=old_ts,
        )
    # New reads
    for _ in range(15):
        detector.observe_read(
            tenant_id="t1", user_id="u1", timestamp=time.time(),
        )
    indicators = detector.flush()
...
def test_detector_flags_cross_tenant_attempt():
    ...
    detector.observe_cross_tenant_attempt(
        actor_tenant="t1", target_tenant="t2", user_id="u1",
    )
    indicators = detector.flush()
...
def test_detector_ignores_same_tenant_as_cross_tenant():
    ...
    detector = BreachDetector()
    detector.observe_cross_tenant_attempt(
        actor_tenant="t1", target_tenant="t1",
    )
    assert detector.flush() == []
...
def test_detector_flags_unauthorized_export():
    ...
    detector.observe_unauthorized_export(
        ...
        export_tool="audit-export", record_count=5000,
    )
    indicators = detector.flush()
...
def test_detector_flush_clears_queue():
    ...
    detector = BreachDetector()
    detector.observe_cross_tenant_attempt(
        actor_tenant="t1", target_tenant="t2",
    )
...
def test_classifier_marks_cross_tenant_as_breach():
    from largestack._compliance.dpdp_breach import (
        BreachClassifier, BreachIndicator,
    )
    cls = BreachClassifier()
    ind = BreachIndicator(
        kind="cross_tenant", detected_at=time.time(),
        tenant_id="t1", record_count=10,
    )
...
def test_classifier_marks_system_intrusion_as_critical():
    ...
    cls = BreachClassifier()
    ind = BreachIndicator(
        kind="system_intrusion", detected_at=time.time(),
        tenant_id="t1",
    )
    result = cls.classify(ind)
...
def test_classifier_after_hours_alone_not_a_breach():
    ...
    cls = BreachClassifier()
    ind = BreachIndicator(
        kind="after_hours", detected_at=time.time(),
        tenant_id="t1", record_count=200,
    )
    result = cls.classify(ind)
...
def test_classifier_mass_read_severity_scales_with_records():
    ...
    cls = BreachClassifier()
    small = cls.classify(BreachIndicator(
        kind="mass_read", detected_at=time.time(),
        tenant_id="t1", record_count=2000,
    ))
    huge = cls.classify(BreachIndicator(
        kind="mass_read", detected_at=time.time(),
        tenant_id="t1", record_count=200_000,
    ))
...
def test_dpb_notification_includes_required_fields():
    from largestack._compliance.dpdp_breach import (
        BreachClassifier, BreachIndicator, render_dpb_notification,
    )
    cls = BreachClassifier()
    ind = BreachIndicator(
        kind="cross_tenant", detected_at=time.time(),
        tenant_id="t1", record_count=500,
...
def test_principal_notification_uses_plain_language():
    from largestack._compliance.dpdp_breach import (
        BreachClassifier, BreachIndicator, render_principal_notification,
    )
    cls = BreachClassifier()
    classification = cls.classify(BreachIndicator(
        kind="unauthorized_export", detected_at=time.time(),
        tenant_id="t1", record_count=5000,
...
async def test_logging_notifier_records_sends():
    from largestack._compliance.dpdp_breach import (
        BreachClassifier, BreachIndicator, LoggingNotifier,
    ...
    notif = render_dpb_notification(
        classification,
        organisation_name="X", contact_email="x@x.in",
    )
