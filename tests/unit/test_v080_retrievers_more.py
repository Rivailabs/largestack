...
async def test_auto_merging_merges_when_threshold_met():
    ...
    async def pl(pid):
        return {"id": pid, "content": f"parent {pid}"}
...
async def test_recursive_retrieve_follows_references():
    ...
    async def initial(q, k):
        return [{"id": "doc1", "score": 0.9, "metadata": {"references": ["doc2"]}}]
    ...
    out = await recursive_retrieve(
        "q",
        initial_retriever=initial,
        follow_lookup=follow,
        k=5,
        depth=2,
    )
    ids = [d["id"] for d in out]
...
def test_time_weighted_rerank_boosts_recent():
    ...
    now = 1_000_000.0
    results = [
        {"id": "old", "score": 0.9, "metadata": {"timestamp": now - 365 * 24 * 3600}},  # 1 year old
        {"id": "new", "score": 0.7, "metadata": {"timestamp": now - 60}},  # 1 minute old
    ]
...
async def test_document_summary_retrieve():
    ...
    async def summary_ret(q, k):
        return summaries

    async def full_lookup(did):
        return full_docs.get(did)

    out = await document_summary_retrieve(
        "q",
        summary_retriever=summary_ret,
        full_doc_lookup=full_lookup,
        k=5,
    )
    assert len(out) == 2
