...
def test_chunker_recursive():
    ...
    chunks = c.chunk(
        "First sentence here. Second sentence here. Third sentence is longer than expected."
    )
    assert len(chunks) >= 1
...
def test_bm25():
    bm25 = BM25()
    bm25.index(
        ["python programming language", "java enterprise framework", "rust systems programming"]
    )
...
def test_pipeline():
    rag = RAGPipeline(
        documents=["LARGESTACK costs 299 per year.", "LARGESTACK supports 15 providers."]
    )
