...
def test_decorator_concurrent_runs_isolated_contexts():
    ...
    seen_users = []
    agent = Agent[Deps, str]("openai/gpt-4o-mini", deps_type=Deps, instructions="x")
...
def test_audit_logs_failed_status():
    ...
    assert 'audit.log( "agent.run", run_status,' in flat_src
...
def test_gateway_fallback_routes_via_retry():
    ...
    assert "await self._retry( fb, messages, fallback_model" in flat_src
