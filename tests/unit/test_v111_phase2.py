...
def test_bm25_stemming_matches_inflections():
    ...
    rag = create_rag(
        ["Refunds are available within 30 days.", "Warranty covers 12 months."], top_k=1
    )
    assert "30 days" in rag.retrieve("refund policy")[0]["text"]  # refund→refund(s)
    assert (
        "Warranty" in rag.retrieve("warranties")[0]["text"]
    )  # warranties→warranti? still matches warranty
...
def test_build_native_params_ollama_format():
    ...
    params = build_native_params(
        "ollama/qwen2.5:0.5b", {"type": "object", "properties": {"x": {"type": "string"}}}
    )
