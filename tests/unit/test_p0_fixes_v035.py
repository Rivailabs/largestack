...
def test_vault_uses_pbkdf2_not_single_sha256():
    ...
    assert "iterations=600_000" in src or "iterations=600000" in src, (
        "must use OWASP-recommended iteration count"
    )
...
def test_sso_production_refuses_unsigned_jwt(monkeypatch):
    ...
    sso = SSOProvider(provider="oidc", client_id="cid", client_secret="cs", jwks_url="")  # no JWKS
...
def test_sso_dev_allows_unsigned_jwt_with_warning(monkeypatch):
    ...
    sso = SSOProvider(provider="oidc", client_id="cid", client_secret="cs", jwks_url="")
    import base64, json
