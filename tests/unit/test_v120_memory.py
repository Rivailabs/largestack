...
async def test_build_context_assembles_all_sections():
    ...
    mgr = LongTermMemoryManager(tenant_id="t1", user_id="u1")
    await mgr.add_core("Persona: helpful India fintech assistant", tag="persona")
...
async def test_extract_facts_parses_json_array():
    ...
    judge.run = AsyncMock(
        return_value=MagicMock(
            content=(
                '[{"content": "User prefers Hindi", "scope": "semantic", "tag": "preferences"}]'
            )
        )
    )
...
async def test_extract_facts_caps_at_max():
    ...
    judge = MagicMock()
    many = [{"content": f"fact {i}", "scope": "semantic"} for i in range(20)]
    import json as _j
...
async def test_extract_facts_normalizes_invalid_scope():
    ...
    judge = MagicMock()
    judge.run = AsyncMock(
        return_value=MagicMock(content=('[{"content": "x", "scope": "BOGUS_SCOPE"}]'))
    )
    facts = await extract_facts(judge, turn="x")
...
async def test_extract_and_store_writes_to_archival_by_default():
    ...
    judge.run = AsyncMock(
        return_value=MagicMock(
            content=(
                '[{"content": "User is in Bengaluru", "scope": "semantic", "tag": "location"}]'
            )
        )
    )
