...
def test_swarm_handoff_blocked_by_allowlist():
    ...
    triage = MockAgent(
        "triage", "[HANDOFF:admin]", handoff_to=["billing"]
...
def test_router_basic():
    ...
    r = Router(
        classifier=classifier, routes={"billing": billing, "technical": tech}, default="billing"
    )
...
def test_router_default_fallback():
    ...
    r = Router(
        ...
        routes={"general": fallback, "other": MockAgent("o")},
        default="general",
    )
...
def test_router_pattern_matching_fallback():
    ...
    r = Router(
        classifier=classifier,
        routes={"billing": MockAgent("b", "OK"), "tech": MockAgent("t", "Tech")},
    )
