...
def test_all_errors_inherit_largestack_error():
    test_cases = [
        ProviderError("test"),
        ProviderTimeoutError("p", 30),
        ProviderAuthError("p"),
        ProviderRateLimitError("p", 60),
        AllProvidersFailedError(["p1", "p2"]),
        BudgetExceededError(5.01, 5.0),
        LoopDetectedError(25),
        ContextWindowExceededError("model", 100000, 128000),
        GuardrailBlockedError("pii", "detected"),
        KillSwitchActivatedError(),
        ToolExecutionError("tool", "error"),
        ToolPermissionError("agent", "tool"),
        LicenseRequiredError(),
    ]
    for err in test_cases:
        assert isinstance(err, LargestackError), (
            f"{type(err).__name__} doesn't inherit LargestackError"
        )
...
def test_error_has_code():
    ...
    for err in test_cases:
        ...
        assert err.error_code.startswith("LARGESTACK_"), (
            f"{type(err).__name__} code doesn't start with LARGESTACK_"
        )
...
def test_gateway_unknown_provider():
    ...
    try:
        asyncio.run(
            gw.chat(model="nonexistent/model", messages=[{"role": "user", "content": "hi"}])
        )
...
def test_payment_invalid_json():
    ...
    pw = PaymentWebhook(
        ...
        signing_secret="",
        allow_unsigned=True,
        db_path=os.path.join(tempfile.mkdtemp(), "l.db"),
    )
...
def test_payment_unknown_event():
    ...
    payload = json.dumps(
        {"meta": {"event_name": "unknown_event"}, "data": {"attributes": {}}}
    ).encode()
