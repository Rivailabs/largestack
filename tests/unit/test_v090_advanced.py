...
async def test_faithfulness_metric():
    ...
    judge = MagicMock()
    judge.run = AsyncMock(
        return_value=MagicMock(content='{"score": 9, "reasoning": "all claims verified"}')
    )
...
async def test_answer_relevance_metric():
    ...
    judge = MagicMock()
    judge.run = AsyncMock(
        return_value=MagicMock(content='{"score": 8, "reasoning": "addresses question well"}')
    )
...
async def test_eval_handles_unparseable_judge_response():
    ...
    judge = MagicMock()
    judge.run = AsyncMock(return_value=MagicMock(content="I think this answer is okay, like 7/10."))
