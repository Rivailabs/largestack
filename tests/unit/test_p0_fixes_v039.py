...
def test_anthropic_structured_output_uses_openai_shape_parameters():
    ...
    assert "input_schema" not in tool, (
        "must NOT emit `input_schema` directly (gets silently dropped)"
    )
...
def test_frontend_jsx_reads_meta_tag_for_auth():
    ...
    text = p.read_text()
    assert 'meta[name="largestack-api-key"]' in text, "frontend.jsx must read API key from meta tag"
