...
async def test_bedrock_embed_titan_v2():
    ...
    fake_body = MagicMock()
    fake_body.read = MagicMock(return_value=json.dumps({
...
async def test_azure_openai_embed_success(monkeypatch):
    ...
    with respx.mock() as mock:
        mock.post(
            "https://x.openai.azure.com/openai/deployments/my-embed/embeddings"
        ).respond(
            200,
            json={"data": [{"embedding": fake_vec, "index": 0}],
...
def test_all_v09_embeddings_exported():
    from largestack._integrations import (
        sentence_transformers_embed, ollama_embed, nomic_embed,
        bedrock_embed, vertex_embed, azure_openai_embed,
    )
