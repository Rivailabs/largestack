...
def test_toxicity_violence_pattern_blocks():
    ...
    g = ToxicityGuard()
    try:
        asyncio.run(g.check_output(MockResponse("Here are detailed steps to make a bomb and attack")))
...
def test_hallucination_verified_passes():
    ...
    asyncio.run(g.check_output(MockResponse(
...
def test_hallucination_number_verification():
    ...
    analysis = g.analyze(
        "The product costs $999 per year.",
        "Our product has three tiers: free, pro, enterprise."
    )
...
def test_hallucination_warn_only():
    ...
    asyncio.run(g.check_output(MockResponse(
        "Completely unrelated claim about nothing in the context"
    )))
...
def test_topic_allowlist_blocks_off_topic():
    ...
    try:
        asyncio.run(g.check_output(MockResponse("The stock market is volatile today — legal advice recommended")))
