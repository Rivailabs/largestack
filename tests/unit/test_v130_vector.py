...
async def test_vector_store_add_and_search_finds_paraphrase():
    ...
    results = await store.search(
        tenant_id="t1", user_id="u1", query="Bengaluru",
    )
...
async def test_vector_store_isolates_by_tenant():
    ...
    t1_results = await store.search(
        tenant_id="t1", user_id="u1", query="secret",
    )
...
async def test_vector_store_filters_user():
    ...
    results = await store.search(
        tenant_id="t1", user_id="alice", query="grocery",
    )
...
async def test_vector_store_excludes_expired():
    ...
    expired = LongTermMemoryEntry(
        ...
        tier="recall", scope="episodic",
        content="findme content",
        created_at=_t.time() - 1000, ttl_seconds=10,
    )
...
async def test_vector_store_falls_back_when_embed_fails():
    ...
    store = VectorMemoryStore(
        InMemoryLongTermStore(), embedder=bad_embedder,
    )
...
async def test_vector_store_clear_drops_index():
    ...
    results = await store.search(
        tenant_id="t1", user_id="u1", query="test",
    )
    assert results == []
...
async def test_vector_store_reindex_rebuilds_from_backing():
    ...
    for i in range(5):
        await backing.add(LongTermMemoryEntry(
            id=f"e{i}", tenant_id="t1", user_id="u1",
            tier="archival", scope="semantic",
            content=f"content {i}",
        ))
    ...
    results = await store.search(
        tenant_id="t1", user_id="u1", query="content",
    )
    assert len(results) >= 1
