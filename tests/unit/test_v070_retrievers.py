...
async def test_hyde_generates_then_embeds_then_searches():
    ...
    vstore = MagicMock()
    vstore.query = AsyncMock(return_value=[
...
async def test_hyde_returns_empty_when_embedding_fails():
    ...
    out = await hyde_retrieve(query="q", agent=agent, embedder=embedder,
                              vector_store=vstore, k=3)
    assert out == []
...
async def test_hyde_validates_k():
    ...
    with pytest.raises(ValueError):
        await hyde_retrieve(
            query="q", agent=MagicMock(), embedder=AsyncMock(),
            vector_store=MagicMock(), k=0,
        )
