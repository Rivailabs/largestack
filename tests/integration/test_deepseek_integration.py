...
def test_basic_agent():
    ...
    async def run():
        a = Agent(
            name="test",
            instructions="Reply in 1 sentence.",
...
def test_team_sequential():
    ...
    async def run():
        r = Agent(
            name="r",
            instructions="List 1 fact. Max 20 words.",
            llm="deepseek/deepseek-chat",
            cost_budget=0.03,
        )
        w = Agent(
            name="w",
            instructions="Rewrite in 1 sentence.",
            llm="deepseek/deepseek-chat",
            cost_budget=0.03,
        )
...
def test_rag():
    ...
    async def run():
        rag = create_rag(
            documents=["LARGESTACK costs $299/year.", "LARGESTACK has 15 guardrails."],
            chunk_size=100,
        )
        a = Agent(
            name="kb",
...
def test_workflow_dag():
    ...
    async def run():
        ...
        async def step1(state):
            a = Agent(
                name="s1",
                instructions="1 fact. Max 15 words.",
                llm="deepseek/deepseek-chat",
                cost_budget=0.02,
            )
        ...
        async def step2(state):
            a = Agent(
                name="s2",
                instructions="Rephrase. Max 15 words.",
                llm="deepseek/deepseek-chat",
                cost_budget=0.02,
            )
