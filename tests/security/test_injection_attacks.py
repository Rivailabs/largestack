...
def test_no_shell_true_in_subprocess():
    ...
    pattern = re.compile(r"subprocess\.(?:run|Popen|call|check_(?:output|call))\s*\([^)]*shell\s*=\s*True", re.MULTILINE | re.DOTALL)
    found = []
...
def test_run_request_rejects_oversized_task():
    ...
    try:
        ...
        for k in ("LARGESTACK_API_KEY", "LARGESTACK_RATE_LIMIT_DISABLE", "LARGESTACK_MAX_TASK_LENGTH"):
...
def test_cost_budget_negative_rejected():
    ...
    try:
        ...
        a = Agent(name="t", llm="openai/gpt-4o-mini")
        client = TestClient(create_api(a))
        r = client.post("/run", json={"task": "ok", "cost_budget": -1.0},
                       headers={"X-API-Key": "k"})
        assert r.status_code == 422
    finally:
...
def test_max_turns_zero_rejected():
    ...
    try:
        ...
        a = Agent(name="t", llm="openai/gpt-4o-mini")
        client = TestClient(create_api(a))
        r = client.post("/run", json={"task": "ok", "max_turns": 0},
                       headers={"X-API-Key": "k"})
        assert r.status_code == 422
    finally:
        for k in ("LARGESTACK_API_KEY", "LARGESTACK_RATE_LIMIT_DISABLE"):
