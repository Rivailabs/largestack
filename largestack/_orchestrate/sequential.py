...
class SequentialPipeline:
    ...
    async def run(self, task: str, **kw) -> AgentResult:
        ...
        for i, agent in enumerate(self.agents):
            ...
            log.info(
                f"Sequential: stage {i + 1}/{len(self.agents)} — {getattr(agent, 'name', 'agent')}"
            )
            ...
            try:
                ...
                if self.on_error == "skip":
                    ...
                    raise LargestackError(
                        f"Sequential pipeline failed at stage {i + 1}: {e}"
                    ) from e
