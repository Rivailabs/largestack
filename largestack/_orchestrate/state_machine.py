...
class StateMachine:
    ...
    def __init__(
        self, name: str = "state_machine", max_transitions: int = 50, cost_budget: float = 0
    ):
        self.name = name
    ...
    async def run(self, initial_state: dict[str, Any] = None) -> dict[str, Any]:
        ...
        state["_final_node"] = current
        state["_total_steps"] = step
    ...
    def _wrap_if_agent(self, handler: Any, node_name: str) -> Callable:
        if (
            ...
            and hasattr(handler, "instructions")
        ):
            agent = handler
