...
class TenantManager:
    ...
    def register(
        self, tenant_id: str, config: dict = None, name: str = "", tier: str = "free", **kwargs
    ) -> Tenant:
    ...
    def check_rate_limit(self, tenant_id: str) -> tuple[bool, str]:
        ...
        self._rate_tracker[tenant_id] = [
            ts for ts in self._rate_tracker[tenant_id] if now - ts < 60
        ]
    ...
    def list_tenants(self) -> list[dict]:
        return [
            {"id": t.tenant_id, "name": t.name, "tier": t.tier, "active": t.active}
