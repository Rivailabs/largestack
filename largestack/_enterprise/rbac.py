...
ROLES = {
    "admin": {
        "*"
    },  # Wildcard — covers agent.create, agent.delete, etc.  # Wildcard = all permissions
    "operator": {
        "agent.run",
        "agent.read",
        "agent.list",
...
class RBAC:
    ...
    def _init_db(self) -> None:
        ...
        with _sq.connect(self._db_path) as conn:
            ...
            """)
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_rbac_users_updated ON rbac_users(updated_at)"
            )
            conn.commit()
    ...
    def list_users_for_tenant(self, tenant_id: str) -> list[str]:
        ...
        prefix = f"{tenant_id}:"
        return [uid[len(prefix) :] for uid in self._users if uid.startswith(prefix)]

    def add_user(
        self, user_id: str, roles: list[str] = None, custom_permissions: Set[str] = None
    ) -> User:
...
def fastapi_middleware(rbac: RBAC, permission_map: dict):
    ...
    async def middleware(request, call_next):
        ...
        if required:
            ...
            if not user_id:
                return JSONResponse(
                    {"error": "Missing X-User-Id header (authentication required)"}, status_code=401
                )
            if not rbac.check(user_id, required):
                return JSONResponse({"error": f"Forbidden: requires {required}"}, status_code=403)
