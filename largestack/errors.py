...
class ProviderTimeoutError(ProviderError):
    retryable = True
    error_code = "LARGESTACK_PROVIDER_TIMEOUT"
...
class ProviderRateLimitError(ProviderError):
    retryable = True
    error_code = "LARGESTACK_RATE_LIMIT"
...
class AllProvidersFailedError(ProviderError):
    ...
    def __init__(self, tried: list[str]):
        super().__init__(
            f"All providers failed: {', '.join(tried)}", "Add local fallback: llm='ollama/llama3'"
        )
...
class BudgetExceededError(LargestackError):
    ...
    def __init__(self, actual: float, budget: float):
        super().__init__(
            f"Cost ${actual:.4f} exceeded budget ${budget:.2f}",
            f"Increase: cost_budget={budget * 2:.2f}",
        )
...
class LoopDetectedError(LargestackError):
    ...
    def __init__(self, iters: int, reason: str = ""):
        super().__init__(
            f"Agent stuck after {iters} iterations ({reason})",
            "Improve instructions or reduce max_turns",
        )
...
class ContextWindowExceededError(LargestackError):
    ...
    def __init__(self, sent: int, maximum: int, model: str):
        super().__init__(
            f"{sent} tokens > {model} max {maximum}", "Use larger model or enable compression"
        )
...
class GuardrailBlockedError(LargestackError):
    ...
    def __init__(self, guard_type: str, details: str):
        ...
        super().__init__(
            f"Blocked by {guard_type}: {details}", "Adjust config or set action='warn'"
        )
...
class ModelRequestsBlockedError(LargestackError):
    ...
    def __init__(self, model: str = ""):
        super().__init__(
            f"Real model request blocked (model={model!r}). ALLOW_MODEL_REQUESTS is False.",
