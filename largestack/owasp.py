...
OWASP_COVERAGE: tuple[OwaspControl, ...] = (
    ...
    OwaspControl(
        "LLM03", "Supply Chain", LLM_TOP_10, PARTIAL,
        ("SBOM generation (CycloneDX/SPDX, `largestack sbom`)", "CI: pip-audit + bandit + trivy + SBOM artifact",
         "PyPI Trusted Publishing (OIDC, attested)"),
        ("_security/sbom.py", "_cli/commands.py", ".github/workflows/security.yml", ".github/workflows/release.yml"),
    ...
    OwaspControl(
        "LLM04", "Data and Model Poisoning", LLM_TOP_10, PARTIAL,
    ...
    OwaspControl(
        ...
        ("tool_permissions allow/deny", "ToolAccessPolicy (rate+param)", "HITL approval", "max_turns", "cost_budget", "kill-switch"),
        ("_core/tools.py", "_guard/tool_access.py", "_core/hitl.py", "_core/loop_guard.py", "_guard/kill_switch.py"),
    ...
    OwaspControl(
        "LLM07", "System Prompt Leakage", LLM_TOP_10, PARTIAL,
    ...
    OwaspControl(
        "LLM08", "Vector and Embedding Weaknesses", LLM_TOP_10, PARTIAL,
    ...
    OwaspControl(
        "LLM09", "Misinformation", LLM_TOP_10, COVERED,
        ("HallucinationGuard (groundedness)", "CitationEngine", "SecureRAGAgent grounded+cited answers"),
    ...
    OwaspControl(
        "LLM10", "Unbounded Consumption", LLM_TOP_10, COVERED,
        ("cost_budget + BudgetExceededError", "LoopGuard (max_turns/fingerprint/timeout)", "rate limiting"),
    ...
    OwaspControl(
        "ASI02", "Tool / Function Misuse", AGENTIC, COVERED,
        ("ToolAccessPolicy (allow/deny + rate + fullmatch param validation)", "tool_permissions", "approval gating"),
    ...
    OwaspControl(
        "ASI03", "Identity & Privilege Abuse", AGENTIC, COVERED,
    ...
    OwaspControl(
        "ASI06", "Memory & Context Poisoning", AGENTIC, PARTIAL,
    ...
    OwaspControl(
        "ASI-AUDIT", "Repudiation / Insufficient Audit", AGENTIC, COVERED,
    ...
    OwaspControl(
        "ASI-SANDBOX", "Unsafe Code Execution", AGENTIC, PARTIAL,
        ("CodeSandbox (env-scrubbed subprocess + AST import allowlist)", "E2B backend for real isolation"),
...
def render_markdown() -> str:
    ...
    for c in OWASP_COVERAGE:
        ...
        lines.append(f"| {c.id} | {c.name} | {icon.get(c.status, c.status)} | {controls} | {c.notes} |")
    s = owasp_coverage_summary()
    lines.append("")
    lines.append(f"_Summary: {s[COVERED]} covered, {s[PARTIAL]} partial, "
