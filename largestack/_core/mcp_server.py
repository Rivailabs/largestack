...
class MCPServer:
    ...
    def tool(self, fn: Callable = None, *, name: str = None, description: str = None):
        ...
        def decorator(f):
            ...
            self._tools[tname] = {"handler": f, "schema": schema, "description": description or schema["description"]}
            return f
    ...
    async def handle_request(self, request: dict) -> dict:
        ...
        if method == "initialize":
            ...
            tools = [{"name": n, "description": d["description"], 
                       "inputSchema": d["schema"].get("parameters", {})} for n, d in self._tools.items()]
            return self._response(req_id, {"tools": tools})
        elif method == "tools/call":
            return await self._call_tool(req_id, params)
        elif method == "resources/list":
            resources = [{"uri": u, "name": d["name"], "description": d["description"]} for u, d in self._resources.items()]
