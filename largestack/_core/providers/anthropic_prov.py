...
class AnthropicProvider(BaseProvider):
    ...
    async def chat(
        ...
        if "stop_sequences" in kw:
            body["stop_sequences"] = kw["stop_sequences"]
        elif "stop" in kw:
            body["stop_sequences"] = kw["stop"] if isinstance(kw["stop"], list) else [kw["stop"]]
        t0 = time.monotonic()
        try:
            r = await self._c.post("/v1/messages", json=body)
        ...
        for blk in d.get("content", []):
            if blk["type"] == "text":
                ...
                tcs.append(ToolCall(id=blk["id"], name=blk["name"], params=blk.get("input", {})))
        return LLMResponse(
            content=content,
