...
class OllamaProvider(BaseProvider):
    name = "ollama"; supports_tools = False
    def __init__(self, base_url: str = "http://localhost:11434"):
        self._c = httpx.AsyncClient(base_url=base_url, timeout=httpx.Timeout(connect=5, read=300, write=30, pool=5))
    ...
    async def chat(self, messages, model, tools=None, stream=False, temperature=0.7, max_tokens=None, **kw) -> LLMResponse:
        ...
        return LLMResponse(content=d.get("message", {}).get("content", ""), model=mn,
            input_tokens=d.get("prompt_eval_count", 0), output_tokens=d.get("eval_count", 0),
            latency_ms=ms, finish_reason="stop" if d.get("done") else "length")

    async def chat_stream(self, messages, model, **kw) -> AsyncIterator[str]:
        mn = self.get_model(model)
        try:
            async with self._c.stream("POST", "/api/chat", json={"model": mn, "messages": messages, "stream": True}) as r:
                async for line in r.aiter_lines():
                    if line:
                        try:
                            ...
                            continue
                        if ch.get("message", {}).get("content"): yield ch["message"]["content"]
                        if ch.get("done"): break
        except httpx.TimeoutException:
            raise ProviderTimeoutError(self.name, 300)
        except httpx.RequestError as e:
            raise ProviderError(f"{self.name} request error: {e}") from e
