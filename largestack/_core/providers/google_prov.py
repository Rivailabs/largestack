...
class GoogleProvider(BaseProvider):
    ...
    async def chat(
        ...
        if "responseMimeType" in kw:
            body["generationConfig"]["responseMimeType"] = kw["responseMimeType"]
        if "responseSchema" in kw:
            body["generationConfig"]["responseSchema"] = kw["responseSchema"]
        # v0.3.6: also accept snake_case (matches build_native_params output)
        if "response_mime_type" in kw:
            body["generationConfig"]["responseMimeType"] = kw["response_mime_type"]
        if "response_schema" in kw:
            body["generationConfig"]["responseSchema"] = kw["response_schema"]
        if "top_p" in kw:
            body["generationConfig"]["topP"] = kw["top_p"]
        if "top_k" in kw:
            body["generationConfig"]["topK"] = kw["top_k"]
        if "stop" in kw:
        ...
        t0 = time.monotonic()
        try:
            r = await self._c.post(url, json=body)
        ...
        if r.status_code == 429:
            raise ProviderRateLimitError(f"Rate limited by {self.name}")
