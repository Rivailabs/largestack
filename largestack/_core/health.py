...
class AgentHealth:
    ...
    def status(self) -> str:
        ...
        if self.error_rate > 0.1:
            return "degraded"
        if self.silent_failure_rate > 0.3:
            return "degraded"  # Responds but doesn't solve
        return "healthy"
...
class AgentMonitor:
    ...
    def record(
        ...
        if not h:
            h = AgentHealth(agent_name)
            self._agents[agent_name] = h
        if success:
            h.record_success(cost, latency_ms, quality_score=quality_score)
        else:
