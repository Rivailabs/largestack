...
class AgentRegistry:
    ...
    def find(self, capability: str = None, tag: str = None) -> Any | None:
        ...
        for entry in self._agents.values():
            if capability and capability in entry["capabilities"]: return entry["agent"]
            if tag and tag in entry["tags"]: return entry["agent"]
        return None
    ...
    def list_agents(self) -> list[dict]:
        return [{"name": name, "capabilities": list(e["capabilities"]),
                 "llm": e["llm"], "tools": e["tools"]}
                for name, e in self._agents.items()]
    
    def unregister(self, name: str): self._agents.pop(name, None)
    def __len__(self): return len(self._agents)
    def __contains__(self, name: str): return name in self._agents
