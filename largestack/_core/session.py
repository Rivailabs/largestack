...
class SessionStore:
    ...
    def __init__(self, backend: str = "sqlite", db_path: str = "~/.largestack/sessions.db", encryption_key: str = None):
        ...
        if backend == "sqlite":
            ...
            self.db.execute("CREATE INDEX IF NOT EXISTS idx_session ON sessions(session_id, timestamp)")
            self.db.commit()
    
    def _encrypt(self, text: str) -> str:
        if not self._enc_key: return text
        try:
...
class RedisSessionStore:
    ...
    def __init__(self, redis_url: str = "redis://localhost:6379", prefix: str = "largestack:session:"):
        self.prefix = prefix
        try:
            import redis
    ...
    def list_sessions(self, limit: int = 100) -> list[dict]:
        ...
        return [{"session_id": k.decode().replace(self.prefix, ""),
                 "messages": self._r.llen(k)} for k in keys]
...
class PostgresSessionStore:
    ...
    def __init__(self, dsn: str = "postgresql://localhost/largestack"):
        try:
            ...
            with self.conn.cursor() as cur:
                ...
                cur.execute("CREATE INDEX IF NOT EXISTS idx_pg_session ON sessions(session_id, timestamp)")
        except ImportError:
    ...
    def save(self, session_id: str, role: str, content: str, metadata: dict = None):
        import json
        with self.conn.cursor() as cur:
            cur.execute("INSERT INTO sessions (session_id, role, content, metadata) VALUES (%s,%s,%s,%s)",
                (session_id, role, content, json.dumps(metadata or {})))

    def load(self, session_id: str, limit: int = 50) -> list[dict]:
        with self.conn.cursor() as cur:
            cur.execute("SELECT role, content FROM sessions WHERE session_id=%s ORDER BY timestamp DESC LIMIT %s",
                (session_id, limit))
    ...
    def list_sessions(self, limit: int = 100) -> list[dict]:
        with self.conn.cursor() as cur:
            cur.execute("SELECT session_id, COUNT(*) as msgs, MAX(timestamp) as last FROM sessions GROUP BY session_id ORDER BY last DESC LIMIT %s", (limit,))
            return [{"session_id": r[0], "messages": r[1], "last_active": str(r[2])} for r in cur.fetchall()]

    def cleanup_expired(self, max_age_hours: int = 168):
        """Delete sessions older than max_age_hours (default: 7 days)."""
        with self.conn.cursor() as cur:
            cur.execute("DELETE FROM sessions WHERE timestamp < NOW() - INTERVAL '%s hours'", (max_age_hours,))

    def export_session(self, session_id: str) -> list[dict]:
        """Export full session history as JSON-serializable list."""
        with self.conn.cursor() as cur:
            cur.execute("SELECT role, content, timestamp, metadata FROM sessions WHERE session_id=%s ORDER BY timestamp", (session_id,))
            return [{"role": r[0], "content": r[1], "timestamp": str(r[2]), "metadata": r[3]} for r in cur.fetchall()]
