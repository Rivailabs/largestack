...
class EventBus:
    ...
    async def emit(self, event: str, data: dict[str, Any] | None = None):
        ...
        for mw in self._mw.get(event, []):
            try:
                data = await mw(data) if asyncio.iscoroutinefunction(mw) else mw(data)
            except Exception as e:
                log.error(f"MW error {event}: {e}")
        for h in self._h.get(event, []) + self._h.get("*", []):
            try:
                ...
                log.error(f"Handler error {event}: {e}")
