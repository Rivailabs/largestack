...
PROVIDER_MAP = {
    ...
    "local": "local", "openai_compatible": "openai_compatible", "ollama_openai": "ollama_openai",
    "deepseek": "deepseek", "google": "google", "groq": "groq",
    "mistral": "mistral", "bedrock": "bedrock", "azure": "azure",
    "together": "together", "fireworks": "fireworks", "cohere": "cohere",
    "perplexity": "perplexity", "cerebras": "cerebras", "sambanova": "sambanova",
    "openrouter": "openrouter", "xai": "xai", "ai21": "ai21",
    "lepton": "lepton", "nvidia": "nvidia", "anyscale": "anyscale",
    "cloudflare": "cloudflare", "voyage": "voyage", "databricks": "databricks",
    "replicate": "replicate",
...
MODEL_PREFIX_MAP = {
    ...
    "llama": "groq", "mixtral": "mistral", "mistral": "mistral",
    "command": "cohere",
    "sonar": "perplexity", "pplx": "perplexity",
    "grok": "xai",
    "jamba": "ai21",
    "nemotron": "nvidia", "nim": "nvidia",
    "dbrx": "databricks",
}
...
class LLMGateway:
    ...
    def _init_providers(self):
        ...
        if c.openai_compatible_base_url:
            from largestack._core.providers.openai_prov import OpenAIProvider
            self.providers["local"] = OpenAIProvider(c.openai_compatible_api_key or "local", c.openai_compatible_base_url)
        ...
        if c.azure_openai_key and c.azure_openai_endpoint:
            ...
            self.providers["azure"] = AzureOpenAIProvider(c.azure_openai_key, c.azure_openai_endpoint)
        if c.bedrock_region:
            try:
        ...
        for pname, key_field, pclass_path in [
            ("perplexity", "perplexity_api_key", "largestack._core.providers.perplexity_prov.PerplexityProvider"),
            ("cerebras", "cerebras_api_key", "largestack._core.providers.cerebras_prov.CerebrasProvider"),
            ("sambanova", "sambanova_api_key", "largestack._core.providers.sambanova_prov.SambaNovaProvider"),
            ("openrouter", "openrouter_api_key", "largestack._core.providers.openrouter_prov.OpenRouterProvider"),
            ...
            key = getattr(c, key_field, "") or os.environ.get(f"LARGESTACK_{pname.upper()}_API_KEY", "")
            if key:
                import importlib
        ...
        if os.environ.get("LARGESTACK_BENCHMARK_SUBPROCESS", "").lower() not in ("1", "true", "yes"):
            try:
    ...
    def _resolve_provider(self, model: str) -> tuple[str, BaseProvider]:
        if "/" in model:
            ...
            for prefix, provider in MODEL_PREFIX_MAP.items():
                if model.startswith(prefix): pn = provider; break
    ...
    async def chat(self, model: str, messages: list[dict], tools: list[dict]|None=None,
        ...
        pn, prov = self._resolve_provider(model); tried = [pn]
        success = False
        ...
        try:
            ...
            for n, fb in self.providers.items():
                ...
                try:
                    resp = await self._retry(fb, messages, fallback_model, tools, temperature, max_tokens, **kw)
                    success = True
        ...
        # Cost
        resp.cost = self.cost_tracker.calc(resp.model, resp.input_tokens, resp.output_tokens, resp.cached_tokens)
        ...
        try:
            ...
            track_llm_call(resp.model, resp.input_tokens, resp.output_tokens, resp.cost, resp.latency_ms)
        except Exception as _e: log.debug(f"swallowed: {_e}")
        ...
        await bus.emit("llm.response", {"model": resp.model, "cost": resp.cost, "latency_ms": resp.latency_ms, "agent": agent_name})
        return resp

    @retry(stop=stop_after_attempt(2), wait=wait_exponential_jitter(initial=1, max=8, jitter=1),
    ...
    async def _retry(self, prov, msgs, model, tools, temp, max_tok, **kw) -> LLMResponse:
        ...
        try:
            resp = await prov.chat(messages=msgs, model=model, tools=tools, temperature=temp, max_tokens=max_tok, **kw)
            if breaker: breaker.record_success()
            return resp
        except ProviderError as e:
            if breaker: breaker.record_failure(e)
            raise
    ...
    async def stream(self, model: str, messages: list[dict], **kw) -> AsyncIterator[str]:
        ...
        async for tok in p.chat_stream(messages, model, **kw): yield tok

    async def close(self):
        for p in self.providers.values():
            if hasattr(p, "close"): await p.close()
