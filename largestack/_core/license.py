...
def get_machine_fingerprint() -> str:
    ...
    try:
        if platform.system() == "Linux" and os.path.exists("/etc/machine-id"):
            with open("/etc/machine-id") as f: parts.append(f.read().strip())
...
def detect_production() -> tuple[bool, int, dict]:
    ...
    if os.path.exists("/.dockerenv") or os.environ.get("KUBERNETES_SERVICE_HOST"):
        score += 2; details["container"] = True
    cloud_vars = ["AWS_REGION", "GCP_PROJECT", "AZURE_SUBSCRIPTION_ID", "CLOUD_RUN_SERVICE",
                  "ECS_CONTAINER_METADATA_URI", "RENDER", "FLY_APP_NAME", "RAILWAY_ENVIRONMENT"]
    if any(os.environ.get(v) for v in cloud_vars):
        score += 3; details["cloud"] = True
    
    explicit = os.environ.get("LARGESTACK_ENV", "").lower() in ("production", "prod")
    if explicit:
        score += 5; details["env_explicit"] = True
...
class LicenseValidator:
    ...
    def validate(self) -> dict:
        ...
        if not is_prod:
            return {"valid": True, "tier": "community", "mode": "development",
                    "production_score": score, "message": "Development mode — no license required"}
        ...
        try:
            from largestack_license import validate_license as _nl_validate, machine_fingerprint as _nl_fp
            nl = type('nl', (), {'validate_license': staticmethod(_nl_validate), 'machine_fingerprint': staticmethod(_nl_fp)})()
            ...
            if nl.validate_license(payload, sig_hex):
                self._valid = True; self._tier = tier
                return {"valid": True, "tier": tier, "mode": "production",
    ...
    def _validate_python(self) -> dict:
        ...
        try:
            ...
            if time.time() > expiry:
                return self._invalid(f"License expired at {time.strftime('%Y-%m-%d', time.localtime(expiry))}")
        except ValueError:
        ...
        return {"valid": True, "tier": tier_names.get(tier, tier), "mode": "production",
    ...
    def is_valid(self) -> bool:
        if self._valid is None: self.validate()
