...
"""
    ...
    def __init__(
        self, llm: str = "openai/gpt-4o-mini", sandbox: str = "local", max_steps: int = 10
    ):
        self.llm = llm
    ...
    async def run(self, task: str) -> str:
        ...
        for step_n in range(self.max_steps):
            ...
            try:
                resp = await gateway.chat(model=self.llm, messages=messages, temperature=0.2)
            ...
            steps.append(CodeStep(code=code, output=exec_result.stdout, error=exec_result.error))
