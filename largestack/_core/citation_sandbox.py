...
class CitationEngine:
    ...
    def cite(self, answer: str, documents: list[dict]) -> CitedAnswer:
        ...
        for sent in sentences:
            ...
            relevant = [(i, sc) for i, sc in scores if sc >= self.min_overlap]
            relevant.sort(key=lambda x: x[1], reverse=True)

            if relevant:
                # Cite top 1-2 sources
                top_sources = [i for i, _ in relevant[:2]]
                citation_str = " ".join(self.citation_format.format(n=i + 1) for i in top_sources)
        ...
        *,
        ...
        try:
            ...
            try:
                ...
                return CodeExecResult(
                    error="subprocess startup timed out",
                    timed_out=True,
                )
