...
class SemanticCache:
    ...
    def put_exact(self, messages: list[dict], model: str, response: dict, **kw):
        ...
        if len(self._exact) >= self.max_size:
            oldest = min(self._exact, key=lambda k: self._exact[k].get("last_access", self._exact[k]["ts"]))
            del self._exact[oldest]
        self._exact[self._hash(messages, model, **kw)] = {"response": response, "ts": time.time(), "last_access": time.time()}
