...
class GuardrailPipeline:
    ...
    """
    def __init__(self, guards: list = None, action: GuardrailAction = GuardrailAction.BLOCK,
    ...
    @classmethod
    def create(cls, pii: bool = True, injection: bool = True,
               hallucination: bool = False, toxicity: bool = False,
               topic_blocklist: list[str] = None,
               pii_action: str = "redact",
               injection_sensitivity: str = "medium",
               **kwargs) -> "GuardrailPipeline":
        ...
        return create_guardrails(
            pii=pii, injection=injection,
            hallucination=hallucination, toxicity=toxicity,
        ...
        if tasks:
            ...
            for i, r in enumerate(results):
                if isinstance(r, GuardrailBlockedError):
                    if self.action == GuardrailAction.BLOCK: raise r
                    else: log.warning(f"Guard warning: {r}")
                elif isinstance(r, Exception):
                    log.exception(f"Guard {i} raised unexpected exception: {r}")
                    if self.fail_closed:
                        raise GuardrailBlockedError(
                            "guardrail_error",
                            f"Guard {i} crashed: {type(r).__name__}: {r}"
                        )
            ...
            for i, r in enumerate(results):
                if isinstance(r, GuardrailBlockedError):
                    ...
                    if self.fail_closed:
                        raise GuardrailBlockedError(
                            "guardrail_error",
                            f"Output guard {i} crashed: {type(r).__name__}: {r}"
                        )
