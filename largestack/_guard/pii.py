...
FINANCIAL_PATTERNS = {
    "bank_account": re.compile(
        r"\b(?:account|acct)\s*(?:number|no\.?)?\s*[:#-]?\s*\d{9,18}\b", re.I
    ),
    "iban": re.compile(r"\b[A-Z]{2}\d{2}[A-Z0-9]{11,30}\b"),
    "card_context": re.compile(
        r"\b(?:card|debit|credit)\s*(?:number|no\.?)?\s*[:#-]?\s*(?:\d{4}[-\s]?){3}\d{4}\b", re.I
    ),
}
...
class PIIGuard:
    def __init__(self, entities: list[str] | None = None, action: str = "redact"):
        ...
        self.action = action
        self._presidio = None
        presidio_enabled = os.environ.get("LARGESTACK_ENABLE_PRESIDIO_PII", "").lower() in (
            "1",
            "true",
            "yes",
        )
            ...
            if isinstance(c, str):
                if self._detect_external_leak(c):
                    raise GuardrailBlockedError(
                        "sensitive_data", "External sensitive-data leakage detected"
                    )
                ...
                if cfg.mode == GuardrailMode.STRICT or cfg.context == "bfsi":
                    ...
                    if self._detect_any(c):
                        c = self.redact(c)
                    m["content"] = c
                    if self._detect_any(str(m.get("content", ""))) or self._detect_financial(
                        str(m.get("content", ""))
                    ):
    ...
    async def check_output(self, response):
        ...
        if hasattr(response, "content") and isinstance(response.content, str):
            if cfg.mode == GuardrailMode.OBSERVE:
                if (
                    self._detect_any(response.content)
                    or self._detect_secret(response.content)
                    or self._detect_financial(response.content)
                ):
    ...
    def _check_and_block(self, text: str):
        ...
        if self._presidio:
            ...
            if results:
                raise GuardrailBlockedError(
                    "pii", f"PII detected: {[r.entity_type for r in results]}"
                )
            return
        for et, pat in PATTERNS.items():
            if et in self.entities and pat.search(text):
    ...
    def redact(self, text: str) -> str:
        ...
        if self._presidio:
            try:
                ...
                results = self._presidio.analyze(text=text, language="en")
                text = AnonymizerEngine().anonymize(text=text, analyzer_results=results).text
            except ImportError:
                pass
            except Exception as exc:
                log.warning(
                    "Presidio anonymization failed; falling back to regex PII redaction: %s", exc
                )
        ...
        if "ssn" in self.entities:
            text = re.sub(
                ...
                lambda m: f"{m.group(1)}{m.group(2)}[SSN_REDACTED]",
                text,
            )
