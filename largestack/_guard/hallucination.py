...
class HallucinationGuard:
    ...
    def __init__(
        ...
        if mode == "nli":
            ...
            if not nli_enabled:
                log.info(
                    "NLI mode requested but LARGESTACK_ENABLE_NLI_GUARD is not set; using fast mode."
                )
                self.mode = "fast"
            else:
                try:
                    ...
                    log.warning(
                        "NLI mode unavailable (%s). Install/repair transformers + torch "
                        "to enable model-backed hallucination checks; falling back to fast mode.",
                        exc,
                    )
                    self.mode = "fast"
    ...
    async def check_output(self, response) -> None:
        ...
        if analysis["faithfulness"] < self.threshold:
            ...
            msg = (
                f"Faithfulness {analysis['faithfulness']:.2f} < {self.threshold} "
    ...
    def _fast_score(self, claim: str, context: str) -> float:
        ...
        if self.check_numbers:
            ...
            if claim_numbers:
                ...
                scores.append(len(normalized_claim & normalized_context) / len(normalized_claim))
        ...
        if self.check_entities:
            ...
            if claim_entities:
                ...
                scores.append(len(claim_entities & context_entities) / len(claim_entities))

        # 4. Date verification
        claim_dates = set(DATE_RE.findall(claim))
        if claim_dates:
            context_dates = set(DATE_RE.findall(context))
            scores.append(len(claim_dates & context_dates) / len(claim_dates))
