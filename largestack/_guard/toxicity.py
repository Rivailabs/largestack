...
VIOLENCE_INSTRUCTION_PATTERNS = [
    re.compile(
        r"\b(how|steps|tutorial|guide|instruct).{0,30}(kill|murder|attack|harm|hurt|injure)\b", re.I
    ),
...
HATE_PATTERNS = [
    re.compile(
        r"\b(all|every).{0,15}(should be|deserve|must).{0,15}(killed|eliminated|destroyed)\b", re.I
    ),
    ...
    def __init__(
        ...
        if use_ml:
            try:
                ...
                log.warning(
                    "detoxify not installed — falling back to pattern matching. "
