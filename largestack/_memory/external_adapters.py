...
class Mem0Memory:
    ...
    def __init__(self, api_key: str | None = None, user_id: str = "default"):
        self.api_key = (
            api_key or os.environ.get("MEM0_API_KEY") or os.environ.get("LARGESTACK_MEM0_API_KEY")
        )
    ...
    async def add(
        self, content: str, user_id: str | None = None, metadata: dict | None = None
    ) -> bool:
        if not self._available:
            return False
        try:
            self._client.add(
                content, user_id=user_id or self.default_user_id, metadata=metadata or {}
            )
            return True
    ...
    async def search(
        ...
        try:
            results = self._client.search(
                query, user_id=user_id or self.default_user_id, limit=limit
            )
            return [
                MemoryResult(
...
class ZepMemory:
    ...
    def __init__(self, api_key: str | None = None, base_url: str = "https://api.getzep.com"):
        self.api_key = (
            api_key or os.environ.get("ZEP_API_KEY") or os.environ.get("LARGESTACK_ZEP_API_KEY")
        )
    ...
    async def add_message(self, session_id: str, role: str, content: str) -> bool:
        ...
        try:
            await self._client.memory.add(
                session_id=session_id, messages=[{"role": role, "content": content}]
            )
            return True
        except Exception as e:
