...
class ObservationalMemory:
    ...
    async def compress(self, llm_fn=None):
        ...
        if routine:
            ...
            compressed.append({
                "fact": summary, "priority": "routine",
                "timestamp": time.time(), "source_role": "reflector"
            })
    ...
    def _extract_facts(self, text: str, role: str) -> list[dict]:
        ...
        for s in sentences[:5]:  # Max 5 facts per message
            ...
            priority = "routine"
            important_kw = ["must", "always", "never", "important", "critical", "remember", "prefer", "hate", "love"]
            critical_kw = ["deadline", "urgent", "emergency", "password", "secret", "key"]
            if any(kw in s.lower() for kw in critical_kw): priority = "critical"
            elif any(kw in s.lower() for kw in important_kw): priority = "important"
