...
def _resolve_cors_origins() -> list[str]:
    ...
    if raw:
        ...
        if "*" in origins:
            log.warning(
                "LARGESTACK_CORS_ALLOWED_ORIGINS contains '*' — rejected. Use explicit origins."
            )
...
def create_api() -> FastAPI:
    ...
    def overview():
        now = time.time()
        traces_24h = _q(
            TRACE_DB, "SELECT COUNT(*) as n FROM traces WHERE timestamp>?", (now - 86400,)
        )
        cost_24h = _q(
            AUDIT_DB,
        ...
        return {
            ...
            "cost_hourly": [
                {"hour": r["h"], "cost": round(r["c"] or 0, 4), "count": r["n"]}
                for r in cost_hourly
            ],
        }
    ...
    def alerts():
        ...
        if errs and errs[0]["n"] > 10:
            alerts_list.append(
                {"level": "error", "message": f"High error rate: {errs[0]['n']} errors/hour"}
            )
        costs = _q(
            AUDIT_DB, "SELECT SUM(cost) as c FROM audit_log WHERE timestamp>?", (now - 3600,)
        )
        if costs and (costs[0]["c"] or 0) > 10:
            alerts_list.append(
                {"level": "warning", "message": f"High cost: ${costs[0]['c']:.2f}/hour"}
            )
