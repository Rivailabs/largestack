        ...
        async def _run_case(case):
            async with sem:
                try:
                    ...
                    actual = r.content if hasattr(r, "content") else (r.output if hasattr(r, "output") else str(r))
                    ok = self.match_fn(actual, case.expected)
                    return {"input": case.input, "expected": case.expected,
                            "actual": actual[:200], "passed": ok, "error": None}
                except Exception as e:
                    return {"input": case.input, "expected": case.expected,
                            "actual": "", "passed": False, "error": str(e)}
        
        results = await asyncio.gather(*[_run_case(c) for c in cases])
        passed = sum(1 for r in results if r["passed"])
        return EvalReport(total=len(cases), passed=passed,
