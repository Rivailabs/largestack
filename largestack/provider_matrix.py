...
PROVIDER_CAPABILITIES: tuple[ProviderCapability, ...] = (
    ProviderCapability(
        ...
        True,
        status="verified",
        notes="Primary OpenAI chat/tool path.",
    ),
    ProviderCapability(
        "deepseek",
        True,
        ...
        True,
        status="verified",
        notes="OpenAI-compatible provider path; function calling supported by DeepSeek models that expose it.",
    ),
    ProviderCapability(
        "anthropic",
        True,
        ...
        True,
        status="adapter_only",
        notes="Native Anthropic adapter (messages API, tool input_schema mapping, prompt caching). Structurally complete but NOT live-verified end-to-end (no key tested) — run check_connection() with your key.",
    ),
    ProviderCapability(
        "litellm",
        True,
        ...
        True,
        status="partial",
        notes="Gateway to many providers; exact capabilities depend on downstream provider/model.",
    ),
    ProviderCapability(
        "local",
        True,
        ...
        local=True,
        status="partial",
        notes="Generic OpenAI-compatible local endpoint using LARGESTACK_OPENAI_COMPATIBLE_BASE_URL or Ollama /v1 compat mode.",
    ),
    ProviderCapability(
        "ollama",
        True,
        ...
        local=True,
        status="verified",
        notes="Native Ollama /api/chat. Live-verified on qwen2.5:0.5b (chat round-trip, cost=0). Tools go via ollama_openai/.",
    ),
    ProviderCapability(
        "ollama_openai",
        True,
        ...
        local=True,
        status="verified",
        notes="Ollama OpenAI-compatible /v1. Live-verified on qwen2.5:0.5b: chat + function-calling (tool round-trip). Tool reliability still depends on the local model.",
    ),
    ProviderCapability(
        "google",
        True,
        ...
        True,
        status="verified",
        notes="Live-verified on gemini-2.5-flash: chat, function-calling (tools), structured output, cost.",
    ),
    ProviderCapability(
        "groq",
        True,
        ...
        True,
        status="partial",
        notes="OpenAI-compatible provider; verify model-specific tool support.",
    ),
    ProviderCapability(
        "mistral",
        True,
        ...
        True,
        status="partial",
        notes="Verify model-specific tool support.",
    ),
    ProviderCapability(
        "cohere",
        True,
        ...
        True,
        status="partial",
        notes="Chat path present; full tool parsing not guaranteed.",
    ),
    ProviderCapability(
        "bedrock",
        True,
        ...
        True,
        status="partial",
        notes="Requires AWS credentials and provider-specific validation.",
    ),
    ProviderCapability(
        "azure",
        True,
        ...
        True,
        status="partial",
        notes="Azure OpenAI-compatible path.",
    ),
    ProviderCapability(
        "openrouter",
        True,
        ...
        True,
        status="partial",
        notes="OpenAI-compatible aggregator; capabilities vary per routed model.",
    ),
    ProviderCapability(
        "perplexity",
        True,
        ...
        True,
        status="partial",
        notes="Chat/research models; tool support varies.",
    ),
    ProviderCapability(
        "cerebras",
        True,
        ...
        True,
        status="partial",
        notes="OpenAI-compatible path; verify live.",
    ),
    ProviderCapability(
        "sambanova",
        True,
    ...
    ProviderCapability(
        ...
        True,
        status="partial",
        notes="Provider adapter present; verify live.",
    ),
    ProviderCapability(
        "lepton",
        True,
    ...
    ProviderCapability(
        ...
        True,
        status="adapter_only",
        notes="Adapter exists; wire/verify before public claim.",
    ),
    ProviderCapability(
        "cloudflare",
        True,
    ...
    ProviderCapability(
        ...
        True,
        status="adapter_only",
        notes="Embeddings-oriented provider; not chat-agent path.",
    ),
)
...
def tool_capable_providers(*, include_partial: bool = True) -> list[str]:
    ...
    return [
        p.provider
        for p in PROVIDER_CAPABILITIES
        if p.tool_calling and (include_partial or p.status == "verified")
    ]
...
async def check_connection(model: str, timeout: int = 30) -> dict:
    ...
    try:
        ...
        return {
            "provider": provider,
            "model": model,
            "ok": bool(getattr(result, "content", None)),
