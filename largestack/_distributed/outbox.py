...
class OutboxPattern:
    ...
    """
    def __init__(self, db_path: str = "~/.largestack/outbox.db",
                 max_retries: int = 5, retry_base_delay: float = 1.0,
    ...
    def write(self, event_type: str, payload: dict, aggregate_id: str = None,
    ...
    def mark_failed(self, event_id: int, error: str):
        ...
        row = self.db.execute(
            "SELECT retry_count, event_type, payload FROM outbox WHERE id=?",
            (event_id,)
        ).fetchone()
        if not row:
            return
    ...
    def get_dlq(self, limit: int = 100) -> list[dict]:
        ...
        return [{
            "id": r[0], "original_id": r[1], "type": r[2],
            "payload": json.loads(r[3]), "last_error": r[4],
            "retry_count": r[5], "moved_at": r[6]
        } for r in rows]
