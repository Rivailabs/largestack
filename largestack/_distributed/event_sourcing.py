...
class EventStore:
    ...
    def __init__(self, db_path: str = "~/.largestack/events.db", snapshot_every: int = 100):
    ...
    def append(
        ...
        with self._lock:
            ...
            try:
                ...
                raise ConcurrencyError(
                    f"Stream {stream_id} version {new_version} already exists: {e}"
                )
        ...
        with self._lock:
            ...
            for event in events:
                ...
                if sid not in stream_versions:
                    row = self.db.execute(
                        "SELECT MAX(version) FROM events WHERE stream_id=?", (sid,)
                    ).fetchone()
    ...
    def get_by_type(self, event_type: str, limit: int = 1000, since: float = None) -> list[dict]:
    ...
    def reconstruct_state(
        self, stream_id: str, reducer: Callable = None, use_snapshot: bool = True
    ) -> dict:
    ...
    def get_snapshot(self, stream_id: str) -> dict | None:
        row = self.db.execute(
            "SELECT version, state, timestamp FROM snapshots WHERE stream_id=?", (stream_id,)
        ).fetchone()
        if not row:
            return None
        return {
    ...
    def list_streams(self, limit: int = 100) -> list[dict]:
        ...
        return [
            {"stream_id": r[0], "event_count": r[1], "version": r[2], "last_timestamp": r[3]}
            for r in rows
        ]
    ...
    def _fire_subscribers(self, event: dict):
        ...
        for pattern, handler in self._subscribers:
            if (
                pattern == "*"
                or sid == pattern
                or (pattern.endswith("*") and sid.startswith(pattern[:-1]))
            ):
                try:
                    handler(event)
    ...
    def stats(self) -> dict:
        row = self.db.execute(
            "SELECT COUNT(*), COUNT(DISTINCT stream_id), COUNT(DISTINCT event_type) FROM events"
        ).fetchone()
