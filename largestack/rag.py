...
def create_rag(documents: list[str] = None, chunk_size: int = 512, top_k: int = 5,
               dense: bool = False, embed_fn=None, reranker=None) -> RAGPipeline:
    ...
    return RAGPipeline(documents=documents, chunk_size=chunk_size, top_k=top_k,
                       dense=dense, embed_fn=embed_fn, reranker=reranker)
