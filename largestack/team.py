...
class Team:
    ...
    def __init__(
        ...
        fallback_map: dict[str, Any] = None,
        on_complete: Any = None,
        on_agent_error: Any = None,
        **kw,
    ):
        self.agents = agents
    ...
    async def run(self, task: str, context: AgentContext | None = None, **kw) -> AgentResult:
        ...
        if self._on_complete and result:
            try:
                ...
                log.warning(f"callback failed: {_e}")
        return result
    ...
    async def _sequential(self, ctx: AgentContext, **kw) -> AgentResult:
        ...
        for agent in self.agents:
            ...
            if self.cost_budget > 0 and ctx.total_cost >= self.cost_budget:
                log.warning(
                    f"Workflow cost ${ctx.total_cost:.4f} hit budget ${self.cost_budget:.2f} — stopping"
                )
                break
        ...
        return last or AgentResult(
            content="All agents failed or were skipped.",
            agent_name="team",
    ...
    async def _parallel(self, ctx: AgentContext, **kw) -> AgentResult:
        ...
        return AgentResult(
            content="\n\n".join(contents),
            agent_name="team_parallel",
            total_cost=total_cost,
            turns=max((r.turns for r in results if isinstance(r, AgentResult)), default=0),
                ...
                if self._on_error:
                    try:
                        if asyncio.iscoroutinefunction(self._on_error):
                            await self._on_error(agent.name, e)
                        else:
                            self._on_error(agent.name, e)
