...
def assert_pass_rate(check_fn: Callable, results: list, min_rate: float = 0.8):
    ...
    if rate < min_rate:
        raise AssertionError(f"Pass rate {rate:.1%} < required {min_rate:.1%} ({passed}/{len(results)})")
...
class SPRT:
    ...
    """
    def __init__(self, h0_rate: float = 0.7, h1_rate: float = 0.9, alpha: float = 0.05, beta: float = 0.1):
        self.h0 = h0_rate; self.h1 = h1_rate
        self.log_a = math.log(beta / (1 - alpha))     # Lower boundary
        self.log_b = math.log((1 - beta) / alpha)     # Upper boundary
        self.lr_sum = 0.0; self.n = 0
    ...
    def update(self, success: bool) -> str | None:
        ...
        if self.lr_sum >= self.log_b: return "PASS"
        if self.lr_sum <= self.log_a: return "FAIL"
