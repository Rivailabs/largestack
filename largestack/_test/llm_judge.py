...
class LLMJudge:
    ...
    async def evaluate(
        ...
        criteria_text = "\n".join(f"- {c}: {CRITERIA_PROMPTS.get(c, c)}" for c in criteria)
...
Respond with JSON: {{"winner": "A" or "B", "scores_a": {{...}}, "scores_b": {{...}}, "reasoning": "..."}}"""
        ...
        if mitigate_position_bias:
            ...
            swapped_prompt = prompt.replace("Response A:", "Response X:").replace(
    ...
    async def batch_evaluate(self, examples: list[dict], criteria: list[str] = None) -> list[dict]:
        """Evaluate multiple examples. Each dict has 'task' and 'response'."""
        import asyncio

        tasks = [
            self.evaluate(
                ex["task"], ex["response"], criteria, ex.get("reference"), ex.get("context")
            )
            for ex in examples
        ]
