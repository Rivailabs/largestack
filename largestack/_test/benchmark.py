...
class BenchmarkResult:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "mean_ms": round(self.mean_ms, 3), "median_ms": round(self.median_ms, 3),
...
class BenchmarkRunner:
    ...
    async def _bench_async(self, name: str, fn: Callable, iterations: int = None) -> BenchmarkResult:
        iters = iterations or self.iterations
        for _ in range(self.warmup):
            await fn()
        times = []
    ...
    async def bench_guardrails(self):
        ...
        await self._bench_async("guardrail/injection_check",
            lambda: inj.check_input(msgs))
        
        class MockResp:
            content = "Normal safe response about programming"
        await self._bench_async("guardrail/toxicity_check",
            lambda: tox.check_output(MockResp()))
    ...
    async def bench_memory(self):
        ...
        await self._bench_async("memory/semantic_search",
            lambda: sem.search("programming language", k=2))
        ...
        await self._bench_async("memory/graph_query",
            lambda: g.query("A", depth=2))
        await self._bench_async("memory/graph_shortest_path",
            lambda: g.shortest_path("A", "C"))
    ...
    async def bench_rag(self):
        ...
        self._bench_sync("rag/reranking_20docs",
            lambda: reranker.rerank("topic 5 details", docs, top_k=5), iterations=50)
    ...
    def bench_encryption(self):
        ...
        self._bench_sync("security/hash_sha256",
            lambda: EncryptionManager.hash_sha256(plaintext))
    ...
    def bench_anomaly(self):
        ...
        self._bench_sync("observe/anomaly_check",
            lambda: detector.check(random.gauss(100, 10)))
    ...
    def print_report(self):
        ...
        for r in self.results:
            print(f"{r.name:<35} {r.mean_ms:>7.2f}ms {r.p95_ms:>7.2f}ms {r.p99_ms:>7.2f}ms {r.ops_per_sec:>9.0f}")
        print(f"{'='*70}\n")
