...
class RegressionCase:
    def __init__(self, task: str, expected_contains: str = None,
                 expected_not_contains: str = None, max_cost: float = None,
                 max_turns: int = None, required_tools: list[str] = None):
        self.task = task
...
class RegressionSuite:
    ...
    async def run(self, agent, save_results: bool = True) -> dict:
        ...
        for case in self.cases:
            try:
                ...
                if case.expected_contains and case.expected_contains.lower() not in result.content.lower():
                    passed = False; reasons.append(f"Missing: '{case.expected_contains}'")
                if case.expected_not_contains and case.expected_not_contains.lower() in result.content.lower():
                    passed = False; reasons.append(f"Contains forbidden: '{case.expected_not_contains}'")
                if case.max_cost and result.total_cost > case.max_cost:
                    passed = False; reasons.append(f"Cost ${result.total_cost:.4f} > ${case.max_cost:.4f}")
                if case.max_turns and result.turns > case.max_turns:
                    passed = False; reasons.append(f"Turns {result.turns} > {case.max_turns}")
                if case.required_tools:
                    missing = set(case.required_tools) - set(result.tool_calls_made)
                    if missing: passed = False; reasons.append(f"Missing tools: {missing}")

                results["passed" if passed else "failed"] += 1
                results["cases"].append({"task": case.task, "passed": passed,
                    "reasons": reasons, "content": result.content[:200], "cost": result.total_cost})
    ...
    def save(self, path: str = None):
        ...
        data = [{"task": c.task, "expected_contains": c.expected_contains,
                 "expected_not_contains": c.expected_not_contains,
                 "max_cost": c.max_cost, "max_turns": c.max_turns,
                 "required_tools": c.required_tools} for c in self.cases]
