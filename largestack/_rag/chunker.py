...
class Chunk:
    def __init__(self, text: str, metadata: dict[str, Any] = None, index: int = 0):
        self.text = text; self.metadata = metadata or {}; self.index = index
    def __repr__(self): return f"Chunk({len(self.text)} chars, idx={self.index})"
...
class Chunker:
    ...
    def __init__(self, strategy: str = "recursive", chunk_size: int = 512,
                 chunk_overlap: int = 50, separators: list[str] | None = None):
    ...
    def _sentence(self, text: str, meta: dict) -> list[Chunk]:
        ...
        for s in sentences:
            if len(current) + len(s) <= self.chunk_size:
                ...
                if current.strip(): chunks.append(Chunk(current.strip(), meta, len(chunks)))
                current = s
        if current.strip(): chunks.append(Chunk(current.strip(), meta, len(chunks)))
        return chunks
