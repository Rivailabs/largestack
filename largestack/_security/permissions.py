...
class Permissions:
    ...
    data_classifications: list[str] = field(default_factory=lambda: ["public"])  # public/internal/confidential/restricted
    ...
    def check_resource_limits(self, current_cost: float = 0, tool_calls: int = 0,
                              tokens: int = 0, duration_s: float = 0) -> tuple[bool, str]:
        """Check if current resource usage is within limits."""
        if current_cost > self.max_cost_per_run:
            return CheckResult(False, f"Cost limit exceeded: ${current_cost:.4f} > ${self.max_cost_per_run}")
        if tool_calls > self.max_tool_calls_per_run:
            return CheckResult(False, f"Tool call limit exceeded: {tool_calls} > {self.max_tool_calls_per_run}")
        ...
        if duration_s > self.max_duration_seconds:
            return CheckResult(False, f"Duration limit exceeded: {duration_s:.1f}s > {self.max_duration_seconds}s")
        return CheckResult(True, "")
