...
class SecretStore:
    ...
    """
    def __init__(self, backend: str = "env", ttl_seconds: int = 3600,
                 encryption_key: str = None, **config):
        ...
        if encryption_key:
            try:
                ...
                if salt_env:
                    ...
                    salt = hashlib.sha256(b"largestack-vault-v1\x00" + encryption_key.encode()).digest()
                kdf = PBKDF2HMAC(
    ...
    def _aws_fetch(self, key: str, default: str) -> str:
        try:
            import boto3
            client = boto3.client("secretsmanager", region_name=self._config.get("region", "us-east-1"))
    ...
    def _gcp_sm_fetch(self, key: str, default: str) -> str:
        ...
        try:
            ...
            log.warning(
                "GCP Secret Manager backend needs: "
                "pip install google-cloud-secret-manager"
            )
            return default

        project_id = self._config.get("project_id") or os.environ.get(
