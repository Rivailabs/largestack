...
class MTLSManager:
        ...
        try:
            ...
            ca_cert = (
                ...
                .not_valid_after(
                    ...
                    + datetime.timedelta(days=validity_years * 365)
                )
            ...
            allow_stub = os.environ.get("LARGESTACK_ALLOW_INSECURE_MTLS", "").lower() in (
                "1",
                "true",
                "yes",
            )
    ...
    def issue_cert(self, agent_name: str, validity_days: int = None) -> CertInfo:
        ...
        try:
            ...
            cert_info = CertInfo(
                ...
                issued_at=now,
                expires_at=now + validity * 86400,
                serial=serial,
                issuer="largestack-ca-stub",
            )
    ...
    def _issue_real_cert(
        self, agent_name: str, cert_id: str, serial_hex: str, validity_days: int
    ) -> CertInfo:
    ...
    def get_ssl_context(self, agent_name: str):
        ...
        agent_certs = [
            c for c in self._certs.get(agent_name, []) if c.status == "active" and not c.is_expired
        ]
        if agent_certs:
    ...
    def _save_state(self):
        state = {
            "certs": {name: [c.to_dict() for c in certs] for name, certs in self._certs.items()},
    ...
    def _load_state(self):
        ...
        try:
            ...
            for name, certs in state.get("certs", {}).items():
                self._certs[name] = [
                    CertInfo(**{k: v for k, v in c.items() if k in CertInfo.__dataclass_fields__})
                    for c in certs
                ]
    ...
    def stats(self) -> dict:
        ...
        return {
            ...
            "ca_initialized": self._ca_initialized
            or os.path.exists(os.path.join(self.ca_dir, "ca.crt"))
            or os.path.exists(os.path.join(self.ca_dir, "ca.json")),
        }
