...
class SandboxResult:
    def __init__(
        self, stdout: str = "", stderr: str = "", exit_code: int = 0, duration_ms: float = 0
    ):
        self.stdout = stdout
        self.stderr = stderr
...
class CodeSandbox:
    ...
    async def execute(self, code: str, language: str = "python", env: dict = None) -> SandboxResult:
        ...
        if language == "python" and self.allowed_imports is not None:
            ...
            if blocked:
                return SandboxResult(
                    stderr=f"Import blocked: {', '.join(sorted(blocked))}", exit_code=1
                )

        if self.backend == "subprocess":
            log.warning(
                "CodeSandbox: subprocess backend has NO kernel isolation. "
    ...
    async def _subprocess_execute(
        self, code: str, language: str, env: dict = None
    ) -> SandboxResult:
        ...
        try:
            ...
            return SandboxResult(
                stderr=str(e), exit_code=1, duration_ms=(time.monotonic() - t0) * 1000
            )
    ...
    async def _e2b_execute(self, code: str, language: str, env: dict = None) -> SandboxResult:
        ...
        try:
            ...
            log.warning(
                "E2B not installed: pip install e2b-code-interpreter. Falling back to subprocess."
            )
            return await self._subprocess_execute(code, language, env)
