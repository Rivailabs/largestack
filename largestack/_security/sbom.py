...
class SBOMGenerator:
    ...
    def generate(
        self, format: str = "cyclonedx", output_path: str = None, include_transitive: bool = True
    ) -> dict:
    ...
    def _collect_packages(self, include_transitive: bool) -> list[dict]:
        ...
        for dist in importlib.metadata.distributions():
            ...
            is_direct = name.lower().replace("-", "_") in [
                d.replace("-", "_") for d in self.LARGESTACK_DIRECT_DEPS
            ]
            is_optional = any(
            ...
            packages.append(
                {
                    ...
                    "relationship": "direct"
                    if is_direct
                    else ("optional" if is_optional else "transitive"),
