...
app = typer.Typer(name="largestack", help="Largestack AI — Universal Multi-Agent AI Framework", no_args_is_help=True)
console = Console()
...
def init(
    ...
    provider: str = typer.Option("deepseek", help="Provider: deepseek, openai, anthropic, gemini, groq, ollama, multi"),
    rag: str = typer.Option("hybrid", help="RAG mode: none, local, vector, hybrid, graph, sql-vector"),
    ...
    console.print(Panel(
        ...
        f"  largestack test",
        title="Largestack AI", border_style="purple"
    ))
...
def doctor():
    ...
    from pathlib import Path
    import yaml
    checks = [("Python", f"{sys.version_info.major}.{sys.version_info.minor}", sys.version_info >= (3, 10))]
    try:
        from largestack import __version__; checks.append(("LARGESTACK", __version__, True))
    except: checks.append(("LARGESTACK", "not installed", False))
    project_files = (
        "largestack.yaml",
        "providers.yaml",
        "agents.yaml",
    ...
    if any(Path(path).exists() for path in project_files):
        for path in project_files:
            checks.append((path, "present" if Path(path).exists() else "missing", Path(path).exists()))
        parsed = {}
        for path in project_files:
        ...
        missing_workflow_agents = [agent for agent in workflow_agents if agent not in {a.get("id") for a in agents}]
        checks.append(("Workflow agent refs", "valid" if not missing_workflow_agents else f"missing: {', '.join(missing_workflow_agents)}", not missing_workflow_agents))
        ...
        checks.append(("RAG sources", "valid" if not missing_sources else f"missing: {', '.join(missing_sources)}", not missing_sources))
        guard_cfg = (parsed.get("guardrails.yaml") or {}).get("guardrails") or {}
        checks.append(("Guardrail mode", guard_cfg.get("mode", "not set"), bool(guard_cfg.get("mode"))))
        ...
        if strict:
            ...
            approved = provider_bfsi.get("approved_providers") or bfsi_cfg.get("approved_providers") or []
            checks.append(("BFSI approved providers", ", ".join(approved) if approved else "missing", bool(approved)))
            checks.append(("Maker-checker", "enabled" if bfsi_cfg.get("maker_checker") else "missing", bool(bfsi_cfg.get("maker_checker"))))
            checks.append(("Audit", "enabled" if bfsi_cfg.get("audit") else "missing", bool(bfsi_cfg.get("audit"))))
            ...
            dashboard_status = "set" if dashboard_key_set else "not set (required before production)"
            checks.append(("Dashboard auth", dashboard_status, dashboard_key_set or not production))
        checks.append(("Tests", "present" if Path("tests").exists() else "missing", Path("tests").exists()))
    else:
        checks.append(("Project scaffold", "not detected in current directory", True))
    checks.append(("OpenAI key", "set" if os.environ.get("LARGESTACK_OPENAI_API_KEY") else "not set (optional)", True))
    checks.append(("DeepSeek key", "set" if os.environ.get("LARGESTACK_DEEPSEEK_API_KEY") else "not set (optional)", True))
    checks.append(("Anthropic key", "set" if os.environ.get("LARGESTACK_ANTHROPIC_API_KEY") else "not set (optional)", True))
    checks.append(("Docker", "available" if _command_exists("docker") else "not found (optional)", True))
    try:
        import httpx; r = httpx.get("http://localhost:11434/api/tags", timeout=2)
        checks.append(("Ollama", f"running ({len(r.json().get('models',[]))} models)", True))
    except: checks.append(("Ollama", "not running (optional)", True))
...
def explain(
    ...
    valid_sections = {"project", "agents", "workflow", "rag", "guardrails", "tools", "providers", "all"}
    ...
    console.print(Panel(f"[bold]Project:[/bold] {root.resolve()}\n[bold]Section:[/bold] {section}", title="Largestack AI Explain", border_style="purple"))
    if show_all:
    ...
    if agents_path.exists() and section in {"project", "all", "agents"}:
        try:
            ...
            if agents:
                ...
                for agent in agents:
                    console.print(f"  - {agent.get('id', agent.get('name', 'agent'))}: {agent.get('role', agent.get('instructions', ''))}")
    ...
    if tools_path.exists() and section in {"project", "all", "tools"}:
        try:
            ...
            for tool in tools:
                console.print(f"  - {tool.get('id', tool.get('name', 'tool'))}: approval={tool.get('approval', False)}")
            if approvals:
                console.print(f"  approval policy: {', '.join(f'{k}={v}' for k, v in approvals.items())}")
            console.print("  rule: read tools can run; write/delete/send/payment tools need approval")
        except Exception as exc:
    ...
    if guardrails_path.exists() and section in {"project", "all", "guardrails"}:
        try:
            ...
            console.print("  use warn for benchmarks, protect for products, strict for BFSI/customer-sensitive work")
        except Exception as exc:
    ...
    if show_all:
        ...
        console.print("  4. workflow.yaml — choose sequential, parallel, router, supervisor, or debate")
...
def templates_explain(template: str = typer.Argument(..., help="Template name")):
    ...
    from largestack._cli.scaffold import PRODUCT_TEMPLATE_ALIASES, PRODUCT_TEMPLATES, available_templates
    ...
    if not info:
        raise typer.BadParameter(f"Unknown template: {template}. Choose from: {available_templates()}")
    console.print(Panel(f"[bold]{info['title']}[/bold]\n{info['description']}", title="Largestack AI Template", border_style="purple"))
...
def providers(path: str = typer.Argument(".", help="Project path")):
    ...
    console.print(Panel(f"[bold]Project:[/bold] {root.resolve()}", title="Largestack AI Providers", border_style="purple"))
    ...
    console.print(f"  bfsi approved providers: {', '.join(approved) if approved else 'none configured'}")


@app.command()
def graph(
    path: str = typer.Argument(".", help="Project path"),
    write: bool = typer.Option(False, "--write", help="Write workflow_graph.mmd from workflow.yaml"),
...
""".format(
                ...
                nodes="\n  ".join(f"<div class='node'>{idx}. {agent}</div>" for idx, agent in enumerate(agents, 1)),
    ...
    if write:
        ...
        console.print(Panel(str(root.resolve()), title="Largestack AI Workflow Graph", border_style="purple"))
        console.print(f"  mode: {workflow.get('mode', 'not set')}")
...
def knowledge_list(path: str = typer.Argument(".", help="Project path")):
    ...
    console.print(Panel(str(knowledge_dir.resolve()), title="Largestack AI Knowledge", border_style="purple"))
    if not files:
...
def rag_explain(path: str = typer.Argument(".", help="Project path")):
    ...
    missing = [name for name, module in optional.items() if importlib.util.find_spec(module) is None]
...
def rag_inspect(path: str = typer.Argument(".", help="Project path")):
    ...
    console.print(Panel(str(manifest_path), title="Largestack AI RAG Inspect", border_style="purple"))
...
def add_knowledge(source: str = typer.Argument(..., help="File or directory to add"), path: str = typer.Option(".", "--project", "-p")):
    """Alias for `largestack knowledge add`."""
    knowledge_add(source, path)


@add_app.command("integration")
def add_integration(name: str = typer.Argument(..., help="Integration name"), path: str = typer.Option(".", "--project", "-p")):
...
def add_agent(agent_id: str = typer.Argument(..., help="Agent id"), path: str = typer.Option(".", "--project", "-p"), role: str = typer.Option("Describe this agent role.", help="Agent role")):
    ...
    agents.append({"id": agent_id, "role": role, "model": "deepseek/deepseek-chat", "max_retries": 2, "cost_budget": 1.0})
    _write_yaml(cfg_path, data)
    console.print(f"[green]Added agent:[/green] {agent_id}")


@add_app.command("tool")
def add_tool(tool_id: str = typer.Argument(..., help="Tool id"), path: str = typer.Option(".", "--project", "-p"), approval: str = typer.Option("false", help="false or require_approval")):
    ...
    tools.append({"id": tool_id, "module": "app.tools.business_tools", "function": tool_id, "access": "read", "approval": approval_value})
    _write_yaml(cfg_path, data)
...
def mcp_add(
    ...
    command: str | None = typer.Option(None, "--command", help="stdio command for local MCP server"),
    path: str = typer.Option(".", "--project", "-p", help="Project path"),
...
def mcp_test(
    ...
    for server in servers:
        ...
        if server.get("approval") not in {"warn", "require_approval", "block"}:
            issues.append(f"{server.get('name', 'mcp')}: approval should be warn/require_approval/block")
    if issues:
        for issue in issues:
    ...
    async def _connect_all():
        ...
        for server in servers:
            ...
            try:
                ...
                console.print(f"[green]Connected[/green] {server.get('name')}: {len(tools)} tool(s)")
                if suspicious:
                    console.print(f"[yellow]Suspicious tool descriptions:[/yellow] {len(suspicious)}")
            finally:
...
def run(
    ...
    task: str = typer.Option("Hello from LARGESTACK", help="Task/prompt for YAML agents or workflows"),
):
    ...
    if path.suffix.lower() in {".yaml", ".yml"}:
        ...
        if "agents" not in data:
            ...
            result = agent.run_sync(task) if hasattr(agent, "run_sync") else asyncio.run(agent.run(task))
...
@app.command()
def dashboard(
    host: str = typer.Option(None, help="Host to bind. Default: 0.0.0.0 in container, 127.0.0.1 locally"),
    ...
    try:
        import uvicorn; from largestack._dashboard.app import create_app
        uvicorn.run(create_app(), host=host, port=port, log_level="warning")
    except ImportError: console.print("[red]Install: pip install largestack[all][/red]")
...
def migrate_config(path: str = typer.Argument("largestack.yaml", help="Config file path"), write: bool = typer.Option(False, "--write")):
...
def migrate_memory(path: str = typer.Argument("memory.json", help="Memory JSON file path"), write: bool = typer.Option(False, "--write")):
...
def migrate_traces(path: str = typer.Argument("traces.db", help="Trace SQLite DB path"), write: bool = typer.Option(False, "--write")):
...
def new(
    ...
    template: str | None = typer.Option(None, "--template", "-t", help="Beginner template shortcut, e.g. support-ticket"),
):
    ...
    try:
        ...
        console.print(Panel(
            ...
            title=f"Largestack AI: {name}", border_style="purple"
        ))
...
def dev(
    ...
    try:
        ...
        hr_status = "enabled (watchfiles)" if watchfiles_available() else (
            "disabled — install largestack[dev-server] for hot-reload"
        )
    except Exception:
        hr_status = "unknown"
    console.print(Panel(
        ...
        title="LARGESTACK Dev", border_style="purple"
    ))
    try:
        from largestack._cli.dev_server import run_dev_server
        run_dev_server(host=host, port=port)
    except ImportError:
        console.print("[yellow]Install dev dependencies: pip install largestack[dev-server][/yellow]")
