...
results = acheck(
    ...
    vstore.search(tenant_id="t1", user_id="u1", query="Bengaluru", limit=2),
)
if results:
    assert results[0].id == "v1", f"vector search wrong order: {[r.id for r in results]}"
...
specs = check("memory_tool_specs (5 OpenAI-format tools)", lambda: memory_tool_specs(mgr))
assert len(specs) == 5
...
card = check(
    ...
    lambda: AgentCard(
        ...
        skills=[AgentSkill(id="verify_pan", name="Verify PAN", description="Validates PAN format")],
    ),
)

# Sign + verify
signed = check("sign card with HS256", lambda: sign_agent_card_hs256(card, secret="prod-secret"))
ok, _ = check("verify signed card", lambda: verify_agent_card_hs256(signed, secret="prod-secret"))
assert ok, "signed card verification failed"

# Tampering detection
signed.card.name = "evil-agent"
ok, _ = check(
    "tampered card → rejected", lambda: verify_agent_card_hs256(signed, secret="prod-secret")
)
...
server = check(
    "build StreamingA2AServer",
    lambda: StreamingA2AServer(
        card=card,
...
img = check("image_part with bytes", lambda: image_part(data=b"PNG-bytes", media_type="image/png"))
f = check(
    "file_part with bytes",
    lambda: file_part(data=b"PDF-content", media_type="application/pdf", filename="contract.pdf"),
)

# Compose multi-modal message
msg = check("compose multi-modal message", lambda: message_from_parts("user", t, img, f))
...
builder.add_audit_event(agent="kyc", event="pan_verify", payload={"masked_pan": "AAA***1C"})
...
diff = check("compute Studio diff (v0.12 vs main)", lambda: compute_diff(builder_main, builder))
assert "approve" in diff.nodes_added, "diff didn't catch added node"

html = check(
    "render comparison HTML",
    lambda: render_comparison_html(builder_main, builder, label_a="main", label_b="v0.12"),
)
...
cases:
  - name: c1
    input: hi
"""
h = check("hash eval suite (reproducibility)", lambda: hash_suite_yaml(yaml_text))
assert len(h) == 64
...
baseline = {
    ...
    "cases": [{"name": "kyc_pan", "passed": True}, {"name": "kyc_aadhaar", "passed": True}],
}
current = {
    "summary": {"pass_rate": 0.87, "passed": 42, "total": 48},
    "cases": [{"name": "kyc_pan", "passed": True}, {"name": "kyc_aadhaar", "passed": False}],
}

delta = check("compute eval delta", lambda: compute_eval_delta(baseline, current))
assert delta.is_overall_regression
assert any(r.name == "kyc_aadhaar" for r in delta.regressions)

md = check(
    "render PR comment markdown", lambda: render_pr_comment_markdown(delta, suite_name="KYC")
)
...
slack_payload = check("build Slack payload", lambda: build_payload("slack", delta, "KYC"))
assert "blocks" in slack_payload

teams_payload = check("build MS Teams payload", lambda: build_payload("teams", delta, "KYC"))
...
with tempfile.TemporaryDirectory() as td:
    ...
    docs = acheck("load .xlsx (NBFC loan book)", load_xlsx(xlsx_path))
    assert len(docs) == 1
...
chunks = acheck("semantic chunk a paragraph", chunker.chunk(text))
assert len(chunks) >= 1
...
detector = check(
    "build BreachDetector",
    lambda: BreachDetector(
        BreachDetectorConfig(
...
indicators = check("flush indicators (mass-read trigger)", lambda: detector.flush())
...
notif = check(
    "render DPB notification (DPDP §8(6))",
...
def try_deepseek():
    try:
        LiteLLMProvider(model="deepseek/chat", require_india_residency=True)
        return False
    except ValueError:
        return True


assert check("DeepSeek + India residency = REJECTED", try_deepseek), "should have rejected"
...
assert check("Bedrock us-east-1 + India residency = REJECTED", try_us_bedrock)
...
# Burn through basic
acheck("basic tenant: 20 acquires succeed", rl.try_acquire("tenant_basic", cost=20.0))
denied = acheck("basic tenant: 21st acquire denied", rl.try_acquire("tenant_basic", cost=1.0))
assert denied is False

# Premium has independent budget
ok = acheck(
    "premium tenant: still has budget (isolation)", rl.try_acquire("tenant_premium", cost=50.0)
)
assert ok is True
...
compliance:
  ...
  - name: PMLA Rule 9
    section: CDD
""")
    report = check("good agent.yaml → PASS", lambda: run_compliance_check(good))
...
""")
    report = check("China-hosted LLM in financial = FAIL", lambda: run_compliance_check(bad))
    assert not report.passed
    assert any("C050" in f.code for f in report.errors), "should have C050 (China-hosted)"
...
typed = check(
    "TypedAgent[KYCInput, KYCOutput] direct construct",
    lambda: TypedAgent(
        name="kyc-typed",
...
def try_no_name():
    try:
        TypedAgent(name="", model="x", input_model=KYCInput, output_model=KYCOutput)
        return False
    except ValueError:
        return True
...
prompt_msgs = check("typed agent: build prompt from input", lambda: typed._build_prompt(inp))
