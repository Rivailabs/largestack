...
from largestack._memory.long_term import (
    LongTermMemoryManager, InMemoryLongTermStore,
)
...
from largestack._studio import (
    StudioBuilder, NodeSpec, EdgeSpec, ComplianceMarker, AuditEvent,
    MemorySnapshot,
)
...
async def kyc_workflow(
    *,
    ...
    studio.add_audit_event(
        ...
        payload={"masked_pan": pan[:3] + "***" + pan[-1],
                 "verified": pan_res.verified,
                 "aggregator": pan_res.aggregator},
        duration_ms=20.0,
    )
    studio.add_audit_event(
        agent="kyc", event="aadhaar_okyc",
        tenant_id=tenant_id, user_id=customer_id,
        payload={"verified": aadhaar_res.verified,
                 "aggregator": aadhaar_res.aggregator},
        duration_ms=50.0,
    )
    studio.add_audit_event(
        agent="kyc", event="cibil_pull",
        tenant_id=tenant_id, user_id=customer_id,
        payload={"score": cibil_res.raw_response.get("score"),
                 "bureau": "CIBIL"},
        duration_ms=80.0,
    )
    ...
    detector.observe_read(
        tenant_id=tenant_id, user_id=customer_id, record_count=1,
    )

    # ---- Stage 4: decision ----
    all_verified = (
        pan_res.verified
        and aadhaar_res.verified
        and cibil_res.verified
    )
    cibil_score = cibil_res.raw_response.get("score", 0)
    decision = (
        "approved" if all_verified and cibil_score >= 700
        else "manual_review" if all_verified and cibil_score >= 600
        else "rejected"
    )
...
async def main():
    ...
    memory = LongTermMemoryManager(
        tenant_id="nbfc_premium", user_id="agent_kyc",
    ...
    for src, dst in [
        ("intake", "rate_gate"), ("rate_gate", "pan"),
        ("rate_gate", "aadhaar"), ("rate_gate", "cibil"),
        ("pan", "decide"), ("aadhaar", "decide"),
        ("cibil", "decide"), ("decide", "end"),
    ]:
    ...
    cases = [
        {"customer_id": f"C{i:03d}",
         "pan": f"AAACR{i:04d}C",
         "aadhaar_otp_token": f"OTP-{i:06d}"}
        for i in range(50)
    ]
    cases.append({  # one bad PAN to test rejection path
        "customer_id": "C999", "pan": "INVALID",
    ...
    for c in cases:
        try:
            r = await kyc_workflow(
                ...
                memory=memory, studio=studio,
                rate_limiter=rate_limiter, detector=detector,
            )
            results.append(r)
    ...
    other_memory = LongTermMemoryManager(
        tenant_id="nbfc_basic", user_id="other_agent",
    )
    ...
    if indicators:
        ...
        for ind in indicators:
            ...
            print(f"     {ind.kind}: {cls.severity} "
                  f"(notify DPB: {cls.must_notify_dpb})")
    else:
    ...
    studio.set_memory_snapshot(MemorySnapshot(
        tenant_id="nbfc_premium", user_id="agent_kyc",
        core_count=1, recall_count=0, archival_count=archival_count,
...
""")
    ...
    if report.passed:
        print(f"  ✓ agent.yaml passes all DPDP/RBI/PMLA checks")
    else:
        print(f"  ✗ compliance failures:")
