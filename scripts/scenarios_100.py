...
def s010():
    ...
    a = Agent(name="x", instructions="x", llm="openai/gpt-4o-mini", cost_budget=1.0)
    with a.override(model=TestModel()):
        r = asyncio.run(a.run("hi"))
...
def s033():
    ...
    a = Agent(name="x", instructions="x", llm="openai/gpt-4o-mini", tools=[boom])
...
def s039():
    ...
    a = Agent(name="x", instructions="x", llm="openai/gpt-4o-mini", tools=[t1, t2, t3])
...
def s054():
    ...
    g = create_guardrails(pii=True, injection=True, injection_sensitivity="high")
...
def s068():
    ...
    bm25 = BM25()
    bm25.index(["machine learning is great", "deep learning models", "natural language processing"])
...
def s089():
    ...
    cfg = LangfuseConfig(public_key="pk", secret_key="sk", enable=False, allow_non_india_host=True)
...
def s098():
    ...
    assert all([AgentCard, callable(text_part), callable(image_part), callable(file_part)])
...
if __name__ == "__main__":
    print("=" * 72)
    print("  LARGESTACK 1.0.0 — 100-scenario validation suite")
    print("=" * 72)
    print()
