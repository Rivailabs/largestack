...
def build_mode_1_studio() -> StudioBuilder:
    ...
    events = [
        ("riva-intake", "intake.received", {"request": "build me a KYC agent", "tier": "pro"}, 12),
        ("riva-planner", "plan.created", {"steps": 7, "estimated_cost_usd": 0.43}, 380),
        (
            "riva-kyc-specialist",
            "specialist.started",
            {"model": "bedrock-claude-3-haiku", "region": "ap-south-1"},
            18,
        ),
        (
            "riva-kyc-specialist",
        ...
        (
            "riva-validator",
            "validation.layer_3_pii",
            {"passed": True, "patterns_redacted": ["AADHAAR", "PAN", "phone"]},
            28,
        ),
        (
            "riva-validator",
            "validation.layer_7_residency",
            {"passed": True, "region": "ap-south-1"},
            8,
        ),
        (
            "riva-validator",
            "validation.layer_12_lawful_basis",
            {"passed": True, "basis": "contract"},
            14,
        ),
        (
            "riva-validator",
            "validation.layer_18_rbi_retention",
            {"passed": True, "retention_years": 8},
            22,
        ),
        ("riva-eval", "eval.passed", {"similarity": 0.91, "rubric": 8.4, "verified": True}, 240),
        (
            "riva-billing",
            "razorpay.captured",
            {"amount_inr": 4999, "payment_id": "pay_QkLm9pXr2N"},
            850,
        ),
        ("riva-audit", "chain.sealed", {"events": 11, "hash_chain_position": 4127}, 6),
    ]
    for agent, event, payload, dur in events:
        b.add_audit_event(
            agent=agent, event=event, payload=payload, duration_ms=float(dur), tenant_id=tenant
        )

    for c in [
        ComplianceMarker(name="DPDP_Act_2023", section="Section 6", notes="purpose: agent_build"),
        ComplianceMarker(name="DPDP_Act_2023", section="Section 7", notes="basis: contract"),
        ComplianceMarker(
            name="RBI MD-NBFC-D", section="Annex IV", notes="multi-tenant segregation"
        ),
...
def build_mode_2_studio() -> StudioBuilder:
    ...
    events = [
        ("riva-cli", "cli.invoked", {"command": "riva new agent --type kyc --customer acme"}, 35),
        (
            "riva-scaffold",
            "scaffold.generated",
            {"files_created": 5, "loc": 142, "venv": ".venv-riva"},
            220,
        ),
        (
            "riva-vscode-ext",
            "lint.suggestion",
            {"file": "tools.py", "line": 14, "hint": "add @retry(max_attempts=3) for network call"},
            8,
        ),
        ("developer", "tool.implemented", {"file": "tools.py", "tool": "verify_pan", "loc": 18}, 0),
        ("riva-test", "tests.run", {"passed": 12, "failed": 0, "duration_ms": 840}, 840),
        (
            "riva-vscode-ext",
            "hint.coverage",
            {
                "file": "tools.py",
                "coverage_pct": 87.5,
                "warning": "no test for pan='INVALID' branch",
            },
            5,
        ),
        ("developer", "test.added", {"file": "test_tools.py", "test": "test_pan_invalid"}, 0),
        ("riva-test", "tests.run", {"passed": 13, "failed": 0, "coverage_pct": 94.2}, 920),
        ("riva-eval", "eval.passed", {"agent_pass_rate": 0.94, "verified": True}, 180),
        (
            "riva-git",
            "commit.created",
            {"sha": "a3f9d12", "files_changed": 3, "branch": "kyc-agent"},
            45,
        ),
    ]
    for agent, event, payload, dur in events:
    ...
    for c in [
        ComplianceMarker(
            name="DPDP_Act_2023", section="Section 6", notes="purpose: agent_assistance"
        ),
        ComplianceMarker(
            name="DPDP_Act_2023", section="Section 7", notes="basis: contract (per-seat)"
        ),
        ComplianceMarker(
...
def build_mode_3_studio() -> StudioBuilder:
    ...
    b.set_memory_snapshot(
        MemorySnapshot(
            tenant_id=tenant,
            user_id="employee-12847@canara",
            core_count=4,
            recall_count=27,
            archival_count=18420,
            core_block_preview=(
    ...
    events = [
        (
            ...
            {
                "user": "employee-12847",
                "roles": ["loan_officer", "branch_lead"],
                "tenant_scoped": True,
                "verified": True,
            },
            14,
        ...
        (
            ...
            "residency.check",
            {
                ...
                "region": "us-east-1",
                "verified": False,
                "warning": "vendor outside India — flagged",
            },
            8,
        ),
        (
            "riva-policy",
            "policy.evaluate",
            {"policy": "block_non_india_for_PII", "result": "redact_payload"},
            22,
        ),
        (
            "riva-intercept",
            "tool.invoked",
            {"tool": "loan-classifier", "input_redacted": True, "pii_fields_masked": 3},
            12,
        ),
        (
            "legacy-loan-classifier",
            "legacy.executed",
            {"model": "internal-xgboost-v3", "result": "approve", "confidence": 0.87},
            380,
        ),
        (
            "riva-audit",
            "chain.appended",
            {
                ...
                "hash_chain_position": 18421,
                "merkle_root": "0x9f4e2c1a8b7d3f0e",
                "actor": "employee-12847",
            },
            6,
        ),
        (
            "riva-decision",
            "decision.allowed",
            {"reason": "all checks passed", "duration_ms_total": 447},
            1,
        ),
    ]
    for agent, event, payload, dur in events:
    ...
    for c in [
        ComplianceMarker(
            name="DPDP_Act_2023", section="Section 6", notes="purpose: lending_decision"
        ),
        ComplianceMarker(
            name="DPDP_Act_2023",
            section="Section 7",
        ...
        ComplianceMarker(
            name="DPDP_Act_2023", section="Section 11", notes="erasure trigger to wrapped tools"
        ),
        ComplianceMarker(
            name="RBI MD-NBFC-D", section="Annex IV", notes="data segregation enforced"
        ),
        ComplianceMarker(
