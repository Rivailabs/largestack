...
from largestack import Agent, Team, Workflow, Orchestrator, tool, create_guardrails
...
async def run_largestack_smoke() -> dict:
    ...
    with agent_tool.override(model=TestModel(call_tools=["safe_tool"], custom_tool_args={"safe_tool": {"query": "refund"}})):
        tool_result = await agent_tool.run("process refund")
